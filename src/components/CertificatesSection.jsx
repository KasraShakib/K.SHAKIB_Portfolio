...
export default function CertificatesSection() {
  ...
  const certificatesData = t('certificates.items', { returnObjects: true }) || [];
  ...
  return (
    <section id="certificates" className="py-24 px-4 sm:px-8 max-w-5xl mx-auto relative overflow-hidden select-none">
      ...
      >
        <span className="text-xs font-semibold tracking-widest text-indigo-400 uppercase bg-indigo-500/10 border border-indigo-500/20 px-3 py-1 rounded-full">
          ...
          <AnimatePresence mode="wait">
            {certificatesData[activeIndex] && (
              <motion.div
                ...
                className="relative p-8 rounded-3xl border border-white/10 bg-gradient-to-b from-white/[0.07] to-transparent backdrop-blur-2xl"
              >
                <div dir={isRtl ? 'rtl' : 'ltr'} className="flex flex-row items-center justify-between gap-4 mb-6 border-b border-white/10 pb-4">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="flex items-center gap-1.5 text-xs bg-indigo-500/10 text-indigo-200 border border-indigo-500/20 px-3 py-1 rounded-xl whitespace-nowrap">
                      <Clock size={14} className="text-indigo-400" />
                      <span>{t('certificates.durationPrefix', 'مدت:')} {certificatesData[activeIndex].duration}</span>
                    </div>
                    <div className="flex items-center gap-1.5 text-xs bg-emerald-500/10 text-emerald-300 border border-emerald-500/20 px-3 py-1 rounded-xl whitespace-nowrap">
          ...
          return (
            <motion.div
              ...
              className={`rounded-2xl border transition-all duration-300 overflow-hidden ${
                isOpen 
                  ? 'border-indigo-500/40 bg-gradient-to-b from-indigo-500/10 via-white/[0.05] to-transparent backdrop-blur-xl shadow-[0_10px_30px_rgba(99,102,241,0.15)]' 
                  : 'border-white/10 bg-white/5 backdrop-blur-md'
              }`}
            >
              <div
                onClick={() => setExpandedMobileIndex(isOpen ? -1 : idx)}
                dir={isRtl ? 'rtl' : 'ltr'}
              ...
              >
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-300 whitespace-nowrap">
                    {cert.score}
                  </span>
                ...
                {isOpen && (
                  ...
                  >
                    ...
                    <div className="flex flex-wrap gap-2 mb-3">
                      <div className="flex items-center gap-1 text-[11px] bg-indigo-500/10 text-indigo-200 border border-indigo-500/20 px-2.5 py-0.5 rounded-lg whitespace-nowrap">
                        <Clock size={12} className="text-indigo-400" />
                        <span>{t('certificates.durationPrefix', 'مدت:')} {cert.duration}</span>
                      </div>
                      <div className="flex items-center gap-1 text-[11px] bg-emerald-500/10 text-emerald-300 border border-emerald-500/20 px-2.5 py-0.5 rounded-lg whitespace-nowrap">
                    ...
                    {cert.link && cert.link !== '#' && (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="inline-flex items-center gap-1.5 text-xs font-semibold text-indigo-300 hover:text-white bg-indigo-500/20 border border-indigo-500/30 px-3 py-1.5 rounded-xl transition-all"
                      >
                        <span>{t('certificates.viewBtn', 'مشاهده مدرک رسمی')}</span>
